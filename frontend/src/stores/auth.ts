...
export const useAuthStore = defineStore("auth", () => {
      ...
      if (data?.loginUser) {
        ...
        apolloClient.defaultOptions = {
          ...
          watchQuery: {
            ...
            context: {
              headers: {
                Authorization: `Bearer ${data.loginUser.token}`,
              },
            },
          },
          query: {
  ...
  const logout = () => {
    ...
    // Clear Apollo client cache and reset default headers
    apolloClient.resetStore();
    delete apolloClient.defaultOptions;
  };

  // Initialize Apollo client with token if available
  if (token.value) {
    apolloClient.defaultOptions = {
      watchQuery: {
        context: {
